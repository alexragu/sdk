...
/**
 * The version of the *native* services SDK, e.g. "major.minor (build)".
 */
...
/**
 * The version of the *core* services SDK.
 */
...
export enum AuthenticationErrorProperty {
       /** A property which value describes the reason which caused an {@link AuthenticationErrorCode#AuthenticatorExternalConfigError} report. */
   AuthenticatorExternalConfigErrorReason,
       /** A property which value describes the reason which caused an {@link AuthenticationErrorCode#InvalidInput} report. */
...
/**
 * Communicates either an input response or control response from an authenticator UI session to the SDK.
 * This is a container that can contain either an InputResponseType object **or** a ControlRequest object.
 *
 */
...
/**
 * The id (username) of the user for whom the TOTP session is to be invoked.
 */
   getUserId() : string;
...
/**
 * Create a FingerprintInput object with a provided prompt.
 *
 * @param prompt An intial value for the prompt attribute.
 */
...
/**
 * Create a NativeFaceInput object with a provided prompt.
 *
 * @param prompt An intial value for the prompt attribute.
 */
 ...
 *
 */
export abstract class FidoInputResponse extends InputResponseType { 
 ...
 */
   public static createSuccessResponse(fidoResponse: object) : FidoInputResponse;
 ...
 * @param error An error describing the failure mode for the authentication.
 */
   public static createdFailedResponse(currentAuthenticatorStatus: AuthenticatorDescription, error: AuthenticationError) : FidoInputResponse;
}
/**
 * Represents a successful authentication input using fido authenticator
 *
 */
export class FidoAuthSuccessResponse extends FidoInputResponse { 

/**
 * FIDO response message received from the FIDO client to attest to successful authentication/registration.
 */
   public getFidoResponse() : object;

/**
 * FIDO response message received from the FIDO client to attest to successful authentication/registration.
 */
   public setFidoResponse(fidoResponse : object) : void;
}
/**
 ...
 *
 */
export class FidoAuthFailureResponse extends FidoInputResponse { 
 ...
 */
setAvailableTargets(targets: Array<TargetType>) : void;
}
/**
...
/**
 * Create an OtpInputRequestRsend object.
 */
...
/**
 * Create a MobileApproveInputRequestPolling object.
 */
 ...
 */
setChallenge(challenge: TotpChallenge) : void;
}
/**
...
/**
 * Create an PromotionControlRequest object representing a control request.
 *
...
/**
 * The authenticator description, if this object represents an authenticator.
 */
   public getSelectedAuthenticator() : AuthenticatorDescription;
...
/**
 * Create an AuthenticatorDescription object representing an authenticator.
 *
 ...
 * @return A promise object. On successful result, returns: true to indicate successful completion.
 */
identifyDevice(device: ManagedDevice, clientContext: object | null) : Promise<boolean>;
 ...
 * action in this case is a polling request, intitated by the UI layer following a timeout.
 *
 */
 ...
 * message to the user (title and text), and ticket ID in some format. The UI layer should display
 * this when received.
 *
 */
 ...
 * @param autoExecedTarget If the SDK has automatically triggered OTP transmission to one of the targets, this will identify that target.
 *
 * @return UIAuthenticatorSession object for managing the authentication process with this authenticator.
 */
 ...
 *
 * @param userId User ID for authentication configuration session.
 *
 ...
 *
 * @param userId User ID for authentication promotion session.
 * @param actionContext Information on the policy action currently executing, within which this callback is invoked.
 *
 ...
 *
 * @param userId User ID for device management session.
 *
 ...
 *
 * @param userId User ID for approvals management session.
 *
 ...
 * made on that object.
 *
 * @param userId The username for whom this session is requested.
 ...
 * @param userId Current or different userID to use in next policy invocation. Null value when redirected to anonymous policy.
...
export enum CollectorType {
   ...
   Bluetooth,
       /** Supported modules */
   ExternalSDKDetails,
       /** Supported hw authenticators */
   HWAuthenticators,
       /** Capabilities, such as fingerprint, audiocapture, persistent keys */
   Capabilities,
       /** Fido authenticators aaids. This data is saved to local cache and refreshed once a week. */
   FidoAuthenticators,
       /** Larger sized fingerprint data */
   LargeData,
       /** Local enrollments registration status */
   LocalEnrollments
}
 ...
 *
 * @param userId User ID for which approvals should be displayed to process this request.
 */
...
setEnabledCollectors(enabledCollectors: Array<CollectorType>) : void;
   ...
   /**
 * Set the logging level for the SDK. See the LogLevel enumeration for available logging levels and their meanings.
 * The default log level is LegLevel#Error.
 *
...
setLogLevel(level: LogLevel) : void;
   ...
   /**
 * Set the external logger for this SDK. The external logger is notified about every log message generated by the SDK.
 *
 ...
 *
 * @param userId Username to use for TOTP provisioning query.
 ...
 *
 * @param userId User name to use for the TOTP code generation process.
 ...
 *
 * @param userId The id of the user for whom this QR representation is to be processed.
 ...
 *
 * @param userId User name to use for authentication action
 ...
 *
 * @param userId Username to use for the binding status query
 *
 ...
 * would return true.
 *
 * @return List of user IDs for whom the device is bound.
 */
 ...
 *
 * @return List of user IDs who previously completed a successful login on this device.
 */
 ...
 * that will be saved byt he SDK until a call to logout.
 *
 * As part of policy execution the SDK will invoke the different callbacks in the currently set
 * UI handler in order to interact with the application and user.
 *
 ...
 *
 *
 * @param userId Name of user on the device for which data should be cleared.
 */
